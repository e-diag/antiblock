...
- **Оплата Premium**:
  - Проверить, что в меню покупки премиума отображаются корректные цены из настроек (`premium_usdt` — сумма в TON, `premium_stars`).
  - Для Stars: пройти полный сценарий оплаты через Telegram Stars и убедиться, что премиум активируется.
  - Для TON/xRocket: создать счёт, перейти по ссылке оплаты, убедиться, что по webhook от xRocket премиум активируется, а запись счёта помечается как `paid`.
